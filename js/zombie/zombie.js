(function () {
  ...
  var currentWeapon = '';
  var currentWeaponKey = '';
  var currentWeaponLabel = '';
  ...
  function updateWeaponLabel() {
    ...
    var queryWeapon = params.get('weaponLabel') || '';
    var queryWeaponKey = params.get('weapon') || '';
    ...
    currentWeapon = queryWeapon || (sessionMatch && sessionMatch.weaponLabel) || 'Sin selección';
    currentWeaponKey = queryWeaponKey || (sessionMatch && sessionMatch.weapon) || '';
    currentWeaponLabel = currentWeapon;
    ...
    if (expectedGameId) {
      window.sessionStorage.setItem(ACTIVE_MATCH_KEY, JSON.stringify({
        ...
        weaponLabel: currentWeaponLabel,
        playerName: currentPlayerName,
        playerId: localPlayerId
      }));
    }
    var playerEl = document.getElementById('multiplayer-player');
    var weaponEl = document.getElementById('multiplayer-country');
    if (playerEl) {
      playerEl.textContent = 'Jugador: ' + currentPlayerName;
    }
    if (!weaponEl) return;
    weaponEl.textContent = 'Arma conectada: ' + currentWeaponLabel;
  }

  function bindUI() {
    var btnLobby = document.getElementById('btn-volver-lobby');
    var btnArma = document.getElementById('btn-cambiar-arma');
    ...
    if (btnArma) {
      btnArma.addEventListener('click', function () {
  ...
  function addModeloEnZombie() {
    ...
    assets.loadMultijugadorShowcase(gameScene.scene, function () {
      updateWeaponLabel();
    });
  }
  ...
  function init() {
    ...
    updateWeaponLabel();
    if (window.PintaGolZombieScene && typeof window.PintaGolZombieScene.createScene === 'function') {
      try {
        gameScene = window.PintaGolZombieScene.createScene(canvas);
      } catch (error) {
        console.error(error);
    ...
    if (!hasActiveGame) {
      var weaponEl = document.getElementById('multiplayer-country');
      if (weaponEl) {
        weaponEl.textContent = 'Sin partida activa: vista de pruebas 3D.';
      }
    }
    bindUI();
    if (hasActiveGame) {
